...
* Prefer a formatted string literal, a.k.a. f-string, over an explicit formatting operation or the interpolation operator (`f"v={foo}"` instead of `"v={}".format(foo)` or `"v=%s" % (foo,)`). Use single quote for string constants such as function parameters and dictionary keys (`data['foo']`); double quote for strings displayed to the user by functions like `print`, `log`, `warn`, `assert`. Use double quote for formatted string literals (`f"value is {data['bar']}"`).
