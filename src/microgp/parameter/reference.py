...
class LocalReference(Reference):
    ...
    **Examples:**
    ...
    def _valid_targets(self) -> List[NodeID]:
        assert len(self.individual.graph.raw_nodes[self.node]['frame_path']) >= -self.frames_up, \
            f"Can't go up the requested number of frames"
        frame_path = self.individual.graph.raw_nodes[self.node]['frame_path'][0:self.frames_up]
        vtargets = list()
        if self.allow_backward:
            vtargets += [
                n for n in self.individual.get_predecessors(self.node)
                if self.individual.graph.raw_nodes[n]['frame_path'][0:len(frame_path)] == frame_path
            ]
        if self.allow_self:
            vtargets += [self.node]
