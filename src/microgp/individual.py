...
class GraphWrapper:
    ...
    def __getitem__(self, node: NodeID):
        return self.graph.raw_nodes[node]
    ...
    @property
    def raw_nodes(self):
        RAW_NODES_WRN = "Access to raw nodes is deprecated; use nodes()"
        warnings.warn(RAW_NODES_WRN, DeprecationWarning)
        return self._graph.nodes

    @property
    def raw_edges(self):
        RAW_EDGES_WRN = "Access to raw edges is deprecated; use nodes()"
        warnings.warn(RAW_EDGES_WRN, DeprecationWarning)
...
class Individual(Paranoid, Pedantic):
    ...
    **Examples:**
    ...
    def entry_point(self, new_entry_point: NodeID):
        ...
        assert self.graph.raw_nodes[new_entry_point], "New entry point is not in the individual"
    ...
    def add_node(self, parent_node: Union[NodeID, None], macro: Macro, frame_path: Sequence[Frame]) -> NodeID:
        ...
        self.graph.raw_nodes[new_node]['frame_path'] = frame_path

        # add macro information
        self.graph.raw_nodes[new_node]['macro'] = macro
        self.graph.raw_nodes[new_node]['parameters'] = None
        # that's all
        return new_node
    ...
    def delete_node(self, node_to_delete: NodeID) -> None:
        ...
        """
        assert self.graph.raw_nodes[node_to_delete], "This node is not in the graph"
    ...
    def convert_node_to_string(self, node: NodeID) -> str:
        ...
        """
        vars_ = dict()
        for parameter_name, parameter in self.graph.raw_nodes[node]['parameters'].items():
        ...
        if self.graph.in_degree(node) > 1 and not self.graph.is_head(node):
            ...
            label = ''
        return label + self.graph.raw_nodes[node]['macro'].text.format(**vars_)
    ...
    def get_next(self, node: NodeID) -> NodeID:
        ...
        assert len([to for _, to, key in self.graph.raw_edges(node, keys=True) if key == 'next'
                    ]) <= 1, "Found more than one next"
        # pretty weird use of a generator, but it could be efficient...
        return next((to for _, to, key in self.graph.raw_edges(node, keys=True) if key == 'next'), None)
    ...
    def _initialize_frames(self) -> None:
        ...
        assert all([p for n, p in self.graph.raw_nodes(data='frame_path')]), "One or more nodes have frame_path == None"
        # Calculate paths
        for p in [p for n, p in self.graph.raw_nodes(data='frame_path')]:
    ...
    def randomize_macros(self, uninitialized_nodes: Set['Individual'] = None) -> None:
        ...
        for node in uninitialized_nodes:
            macro = self.graph.raw_nodes[node]['macro']
    ...
    def initialize_macros(self, macro: Macro, node: NodeID) -> None:
        ...
        self.graph.raw_nodes[node]['parameters'] = parameters
    ...
    def link_movable_nodes(self) -> Union[bool, None]:
        ...
        for head in self._unlinked_nodes.keys():
            ...
            if parent:
                ...
                if self.graph.raw_nodes[self._entry_point]["frame_path"] == frame_path:
            ...
            while movable_node_id and movable_node_id != first_outside:
                self.graph.raw_nodes[movable_node_id]["frame_path"] = frame_path
    ...
    def arrange_movable_proc(self, nodes_to_copy_from, source_individual, first_movable_node):
        ...
        while node:
            ...
            self.graph.raw_nodes[node]['frame_path'] = frame_path
            ...
            while node:
                ...
                for label, target in successors:
                    if label == 'next':
                        ...
                        heads.append(target)
                        known_heads.add(target)
        nodes_to_delete = set(self.graph.raw_nodes) - set(visited)
        for node in nodes_to_delete:
            self.delete_node(node)
    ...
    def copy_section_node_structure(self, source_individual: 'Individual', source_head: NodeID = None) -> \
        ...
        while heads:
            ...
            while source_node_id:
                source_node = source_individual.graph.raw_nodes[source_node_id]
    ...
    def fill_nodes_with_parameters(self, source_individual: 'Individual', node_translation: Dict[NodeID, NodeID],
        ...
        while uninitialized_nodes:
            for destination_node_id in uninitialized_nodes:
                ...
                self.graph.raw_nodes[destination_node_id]['parameters'] = parameters
    ...
    def copy_parameters_movable(self, source_individual: 'Individual', new_node_id: NodeID, source_node_id: NodeID):
        ...
        self.graph.raw_nodes[new_node_id]["parameters"] = parameters
...
def get_nodes_in_frame(individual: Individual, frame: Frame, frame_path_limit: int = None) -> List[NodeID]:
    ...
    """
    node_list = list()
    for node, data in individual.graph.raw_nodes("frame_path"):
        if data:
            if frame_path_limit:
...
def get_frames(individual: Individual, section: Section = None, section_name: str = None) -> Set[Frame]:
    ...
    frames = set()
    for path in (p for k, p in individual.graph.raw_nodes('frame_path')):
        if path:
            for frame in path:
...
def get_macro_pool_nodes_count(individual: Individual, frames: Set[Frame] = None) -> Dict[Frame, int]:
    ...
    """
    frame_count = dict()
    for node_id, value in individual.graph.raw_nodes().items():
...
def check_individual_validity(individual: Individual) -> bool:
    ...
    values = dict()
    for node, data in individual.graph.raw_nodes(data=True):
